...
// Alternative generation backend: the CADFS-2B model, served either locally
// (`npm run model:serve`) or from a scale-to-zero cloud GPU (see
// model_service/modal_app.py — set CADFS_MODEL_URL to the deployed URL).
// Kept SEPARATE from /generate because the model emits older-version,
// non-parametric reconstruction FeatureScript — impressive shape fidelity, but
// not the editable-dialog 2931 features /generate produces.
const CADFS_MODEL_URL = (process.env.CADFS_MODEL_URL || "http://127.0.0.1:8765").replace(/\/+$/, "");
const CADFS_IS_REMOTE = /^https?:\/\/(?!127\.0\.0\.1|localhost)/i.test(CADFS_MODEL_URL);

app.post("/generate-model", async (req, res) => {
  const prompt = String(req.body.prompt || "").trim();
  if (!prompt) return res.status(400).json({ error: "No prompt provided." });
  try {
    // A scale-to-zero cloud GPU may be asleep: the first request pays a cold
    // start (container boot + model load), so allow a much longer timeout and
    // retry once on a cold-start-shaped failure before giving up.
    const timeoutMs = Number(process.env.CADFS_MODEL_TIMEOUT_MS || (CADFS_IS_REMOTE ? 300000 : 120000));
    const attempts = CADFS_IS_REMOTE ? 2 : 1;
    let upstream = null;
    let lastErr = null;
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        upstream = await fetch(`${CADFS_MODEL_URL}/generate`, {
          method: "POST",
        ...
        lastErr = attemptErr;
        if (attempt === attempts - 1) throw attemptErr;
        console.warn(`[/generate-model] attempt ${attempt + 1} failed (${attemptErr.message}); retrying — remote GPU may be cold-starting.`);
      }
    }
    if (!upstream) throw lastErr || new Error("Model request failed.");
    if (!upstream.ok) {
      const body = await upstream.text();
      return res.status(502).json({ error: `Model service returned ${upstream.status}: ${body.slice(0, 300)}` });
    }
    const data = await upstream.json();
    res.json({
      code: data.featurescript || "",
      featureName: "cadfsModelFeature",
      featureLabel: "CADFS-2B Model Output",
      generationMode: "cadfs_2b_local_model",
      elapsedSec: data.elapsedSec,
      warnings: [
        "Generated by the local CADFS-2B model. This is reconstruction-style FeatureScript (older version, no editable dialog parameters) — paste into an Onshape Feature Studio to import.",
      ],
    });
  } catch (err) {
    const connectionish = /fetch failed|ECONNREFUSED|aborted|timeout/i.test(String(err.message));
    const hint = !connectionish
      ? ""
      : CADFS_IS_REMOTE
        ? ` (cloud model at ${CADFS_MODEL_URL} did not respond — it may be cold-starting; try again in ~30s, or check the deployment)`
        : " (is the model server running? start it with: npm run model:serve — or set CADFS_MODEL_URL to a deployed cloud URL)";
    console.error("[/generate-model]", err.message);
    res.status(503).json({ error: `${err.message}${hint}` });
  }
});
