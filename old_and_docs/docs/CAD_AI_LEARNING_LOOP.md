...
Current local diagnosis: the Supabase project is reachable, but the adaptive memory schema is incomplete. `generations` and `shape_knowledge` are visible; `cad_knowledge`, `cad_memory`, `cad_generation_memory_matches`, `cad_feedback_events`, and `cad_memory_pruning_events` still need the migration. The anon key can read and insert allowed rows, but it cannot create missing tables, so the SQL must be applied in Supabase SQL editor or with a privileged Supabase CLI connection.
...
3. `AI.js` adds database memory and FeatureScript doc snippets to the AI prompt.
4. The generated FeatureScript is saved in `generations` with a timestamp.
5. The small popup shows success/failure, timestamp, database save status, memory count, docs count, and example count without blocking the rest of the page.
